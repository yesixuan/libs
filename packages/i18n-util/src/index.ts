...
let excelPath = ''
let langPathes: any[] = []
let key2value: Map<string, string>
let value2key: Map<string, string>

let config: { zhLangPath: string, langPathes: string[], excelPathes: string, tag: string }
...
// 获取语言包中 键、值 项目映射关系
function extractLang(obj, path = '', value2key = new Map(), key2value = new Map()): { value2key: Map<string, string>, key2value: Map<string, string> } {
...
export function mapDir(dir = rootPath, callback = noop, finish = noop) {
...
// 往 excel 里面追加中文
export function appendExcel(words) {
  // const sheets = xlsx.parse(path.resolve(__dirname, 'lang.xlsx'))
  // sheets[0].data.push(
  //   ...words.map(word => [word])
  // )
...
// 根据语言包生成 exel
export function genExcel(pathes = langPathes) {
  const key2values: Map<string, string>[] = pathes.map(p => {
...
// 获取 excel 里面已经翻译好的字段
export function getTransformedWords() {
...
export function getInner$t(dir = './') {
  const words = new Set()
  mapDir(
