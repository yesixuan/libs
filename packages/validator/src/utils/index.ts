...
export function getHandledRuleConfig(ruleConfig: RuleConfig, defaultRules: RuleMap): PureRuleConfig {
  return Object.entries(ruleConfig).reduce(
    (prev: RuleConfig, [key, value]) => {
      prev[key] = value.map(
        ({validator, msg}) => ({
